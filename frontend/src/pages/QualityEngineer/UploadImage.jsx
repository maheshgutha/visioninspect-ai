...
export default function UploadImage() {
  const [activeTab, setActiveTab] = useState("inline"); // Default to requested in-line scanner
  ...
  return (
    <DashboardLayout>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h2 className="text-xl font-bold text-slate-800 mb-1">Product Image Acquisition & Inspection</h2>
          <p className="text-slate-500 text-sm">
            Automated image quality analysis, real-time in-line conveyor scanning, batch processing, and AI defect detection.
          </p>
        </div>
        ...
        <div className="flex items-center gap-1 bg-slate-200/70 p-1 rounded-xl shrink-0 self-start sm:self-auto">
          ...
          >
            🔬 In-Line Conveyor Scanner & Batch
          </button>
          <button
      ...
      {activeTab === "inline" ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
            <h3 className="font-bold text-slate-800 text-base mb-4">Manual Single Product Upload</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-500 text-sm"
              ...
              {preview && (
                <img src={preview} alt="Preview" className="w-full max-h-64 object-contain rounded-lg border border-slate-200 bg-slate-900" />
              )}

              <button
                type="submit"
                disabled={loading}
                className="w-full bg-brand-600 hover:bg-brand-700 text-white font-medium py-2.5 rounded-lg transition-colors disabled:opacity-60 text-sm shadow-md"
              >
